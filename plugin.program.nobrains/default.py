...
BACKUPLOCATION = ADDON.getSetting('path') if not ADDON.getSetting(
    'path') == '' else 'special://home/'
...
DEFAULTPLUGINS = ['metadata.album.universal', 'metadata.artists.universal', 'metadata.common.fanart.tv', 'metadata.common.imdb.com',
    'metadata.common.musicbrainz.org', 'metadata.themoviedb.org', 'metadata.tvdb.com', 'service.xbmc.versioncheck']
...
def index():
	if AUTOUPDATE == 'Yes':
		if wiz.workingURL(WIZARDFILE) == True:
			...
			if ver > VERSION: addFile('%s [v%s] [COLOR red][B][UPDATE v%s][/B][/COLOR]' %
			                          (ADDONTITLE, VERSION, ver), 'wizardupdate', themeit=THEME2)
	...
	if len(BUILDNAME) > 0:
		...
		if version > BUILDVERSION:
		    build = '%s [COLOR red][B][UPDATE v%s][/B][/COLOR]' % (build, version)
		addDir(build, 'viewbuild', BUILDNAME, themeit=THEME4)
		themefile = wiz.themeCount(BUILDNAME)
		if not themefile == False:
			addFile('None' if BUILDTHEME == "" else BUILDTHEME,
			        'theme', BUILDNAME, themeit=THEME5)
	...
	if wiz.platform() == 'android' or DEVELOPER == 'true': addDir(
	    'Apk Installer', 'apk', icon=ICONAPK, themeit=THEME1)
	if not ADDONFILE == 'http://': addDir('Addon Installer',
	                                      'addons', icon=ICONADDONS, themeit=THEME1)
	if not YOUTUBEFILE == 'http://' and not YOUTUBETITLE == '': addDir(
	    YOUTUBETITLE, 'youtube', icon=ICONYOUTUBE, themeit=THEME1)
	addDir('Save Data', 'savedata', icon=ICONSAVE,     themeit=THEME1)
	if HIDECONTACT == 'No': addFile(
	    'Contact', 'contact', icon=ICONCONTACT,  themeit=THEME1)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addFile('Settings', 'settings', icon=ICONSETTINGS, themeit=THEME1)
	if DEVELOPER == 'true': addDir(
	    'Developer Menu', 'developer', icon=ICONSETTINGS, themeit=THEME1)
...
def buildMenu():
	...
	# while pinpassIsCorrect == False:
	# 	pinpass = dialog.numeric(0, 'Enter PIN')
	# 	if pinpass != correctPassword:
	# 		 ok = dialog.notification('NoBrains', 'Verkeerde PIN code!', xbmcgui.NOTIFICATION_INFO, 5000)
	# 	else:
	# 		pinpassIsCorrect = True
...
def viewBuild(name):
	...
	if buildnamestring in name:
		...
		ok = dialog.notification('NoBrains', 'Build niet gevonden', xbmcgui.NOTIFICATION_INFO, 5000)
	buildnamestring = "NoBrains BB TVboxPRO Build"
	pinpassIsCorrect = False
	if buildnamestring in name:
		correctPassword = '7031'
		while pinpassIsCorrect == False:
			pinpass = dialog.numeric(0, 'Enter PIN')
			if pinpass != correctPassword:
				ok = dialog.notification('NoBrains', 'Verkeerde PIN code!', xbmcgui.NOTIFICATION_INFO, 5000)
